...
const DevoteeDashboard = () => {
  ...
  useEffect(() => {
    ...
    fetchPandits(); fetchConversations(); fetchMyBookings(); fetchMyPayments();
    const socket = io('http://localhost:5000', { transports: ['websocket'] });
    socketRef.current = socket;
    socket.on('connect', () => socket.emit('join', { userId: user._id || user.id, role: 'devotee', city: user.city }));
    socket.on('bookingAccepted', (booking) => { setAcceptedBooking(booking); setWaitingBooking(null); setSelectedChatUser(booking.pandit); setActiveTab('chat'); });
    socket.on('bookingLinkUpdated', ({ bookingId, videoLink }) => setBookings(prev => prev.map(b => b._id === bookingId ? { ...b, videoLink } : b)));
  ...
  const handlePayment = async (booking) => {
    ...
    try {
      ...
      const options = {
        ...
        amount: data.amount, currency: 'INR', name: 'PanditJi',
        description: `Payment for ${booking.pujaType}`, order_id: data.orderId,
        handler: async function (response) {
          try {
            const verifyRes = await axios.post('http://localhost:5000/api/payments/verify', {
              razorpay_order_id: response.razorpay_order_id,
              razorpay_payment_id: response.razorpay_payment_id,
              razorpay_signature: response.razorpay_signature, bookingId: booking._id
            }, { headers: { Authorization: `Bearer ${token}` } });
            if (verifyRes.data.success) {
              alert('Payment Successful! You can now chat with the Pandit.');
              setAcceptedBooking(null);
              setSelectedChatUser(booking.pandit); setActiveTab('chat');
            }
      ...
      rzp.open();
    } catch (err) { alert(err.response?.data?.message || 'Error processing payment'); }
    finally { setLoading(false); }
  };
      ...
      <div className="dd-root">
        ...
        <div className="dd-sidebar">
          ...
          <nav className="dd-nav">
            ...
            <NavItem icon={<HeadphonesIcon size={16} />} label="Support" tab="support" activeTab={activeTab} setActiveTab={setActiveTab} />
          </nav>
